  ...
  window.commonJS = function($){
    ...
    window.hulkappsShowCartDiscounts = function(discounts) {
        ...
        discounts.cart.items.forEach(function(item) {
          $(".hulkapps-cart-item-price[data-key='" + item.key + "']").html("<span class='original_price'>" + removeDecimals(item.original_price_format) + "</span>");
          $(".hulkapps-cart-item-line-price[data-key='" + item.key + "']").html("<span class='original_price'>" + removeDecimals(item.original_line_price_format) + "</span>")
        });
        if (discounts.discount_code && discounts.discount_error == 1){
          $(".hulkapps-cart-original-total").html(removeDecimals(discounts.original_price_total));
          ...
          jQuery(".hulkapps-cart-original-total").html(removeDecimals(discounts.original_price_total))
    ...
    window.hulkappsStart = function($, type = null) {
      window.hulkappsc = {};
      window.hulkapps.page_type = type || window.hulkapps.page_type;
...
//                               html += "<label class='hulkapps_check_option'><input type='checkbox' "+selectedopt+" data-option-key='cbm_"+option_id+"' id='"+option_id+"' class='hulkapps_option_child hulkapps_option_"+option_id+"_visible "+opt_class+"' data-price='"+vprice+"'  data-conditional-value='"+ovalue[0].toString().trim()+"' value='"+ovalue[0].toString().trim()+vp1+"'>"+ovalue[0].toString().trim()+vp2+"</label>"
                              html += "<label class='hulkapps_check_option'><input type='checkbox' "+selectedopt+" data-option-key='cbm_"+option_id+"' id='"+option_id+"' class='hulkapps_option_child hulkapps_option_"+option_id+"_visible "+opt_class+"' data-price='"+vprice+"'  data-conditional-value='"+ovalue[0].toString().trim()+"' value='"+ovalue[0].toString().trim()+vp1+"'></label>"
                            });
                            if(default_count > 0){
                              if (min_selection != '0' && max_selection != '0' && data['is_pro']){
                                if (parseInt(default_count) < parseInt(min_selection) || parseInt(default_count) > parseInt(max_selection)){
                            ...
                            $.each(values_json, function(index,ovalue){
                              ...
                              html += "<input type='text' required='" + is_required + "' data-option-key='tb_"+option_id+"' id='"+option_id+"' class='hulkapps_option_child hulkapps_full_width hulkapps_option_"+option_id+" "+opt_class+"' data-price='"+vprice+"' "+maxlength_limit+"><input type='hidden' name='properties["+option_name+"]' class='tb_property_val'>"
                             if(option_limit != undefined && data['is_pro'] && option_limit['character_limit'] != '' && option_limit['character_limit'] != undefined){
